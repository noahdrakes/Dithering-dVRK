...
class example_application:
    ...
    def joint_effort(self):
        ...
        # get current joint torques just to set size
        jf, _ = self.arm.measured_jf()
        print('torque now is', numpy.array(jf))
        effort_joint = numpy.copy(jf)
        # effort_joint.fill(0.0)
        ...
        self.arm.servo_jf(effort_joint)
        self.arm.jaw.servo_jf(numpy.array([0.0]))
        print('measure again and now is', numpy.array(effort_joint))
        ...
        input('press Enter to continue...')
        effort_joint[1] -= 1
        print('after sub 1 is', numpy.array(effort_joint))
        self.arm.servo_jf(effort_joint)

        sleep_rate = self.ral.create_rate(0.2)
        sleep_rate.sleep()
        ...
        sleep_rate = self.ral.create_rate(1)
        sleep_rate.sleep()
        ...
        # sleep_rate = self.ral.create_rate(0.5)
        # sleep_rate.sleep()
