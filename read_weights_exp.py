#!/usr/bin/env python3
"""
Contact Force Estimation from ROS Bag Files
===========================================
...
def low_pass(data, fs, cutoff, order):
    """Butterworth low pass filtering of a signal"""
    nyq = 0.5 * fs
    f_cut = cutoff / nyq
...
# Convert timestamps to seconds and compute sampling frequencies
...
free_jf[:, joint_index] = free_mean
tau_ext_raw = measured_jf - free_jf

tau_ext = np.copy(tau_ext_raw)
tau_ext[:, joint_index] = measured_mean - free_mean


# =========================== FORCE ESTIMATION ============================
estimated_cf     = np.zeros((min_len, 6))
estimated_cf_raw = np.zeros((min_len, 6))

for i in range(min_len):
    J_inv = np.linalg.inv(jacobian[i, :, :])
    estimated_cf[i, :]     = J_inv.T @ tau_ext[i, :]
    estimated_cf_raw[i, :] = J_inv.T @ tau_ext_raw[i, :]

# select the direction of application of the force
estimated_cf = estimated_cf[:, 2]
estimated_cf_raw = estimated_cf_raw[:, 2]


# =========================== VISUALIZATION ===============================
fig, axs = plt.subplots(1, 2)
axs[0].plot(t_meas, measured_jf[:min_len, joint_index], alpha=0.7, label="contact")
axs[0].plot(t_meas, free_jf[:min_len, joint_index], alpha=0.7, label="free space")
axs[0].set_ylabel("Force [N]")
axs[0].set_xlabel("Time [s]")
axs[0].set_title("MEASURED FORCE")
axs[0].legend()
axs[0].grid()
axs[1].plot(t_meas, estimated_cf, color="purple", label="filtered")
axs[1].plot(t_meas, estimated_cf_raw, color="purple", label="raw", alpha=0.5)
...
contact_mask = (t_meas>13) & (t_meas<15)
gt = np.zeros(min_len)
gt[contact_mask] = args.weight

MAE = mean_absolute_error(estimated_cf[contact_mask], gt[contact_mask])
print(f"\nMean Absolute Error: {MAE:.3f} N")

plt.figure(figsize=(7, 2))
plt.plot(t_meas - t_meas[0], estimated_cf, color="mediumblue", label="filtered")
plt.plot(t_meas - t_meas[0], estimated_cf_raw, color="mediumblue", label="raw", alpha=0.3)
plt.plot(t_meas - t_meas[0], gt, color='black', label='theoretical')
plt.xlabel("Time [s]")
