...
class PositionDithering:
    ...
    def __init__(self, ral, arm_name, dith_ampl, dith_freq, joint_index, period = 0.01):
        ...
        # Joint limits
        self.JOINT_LIMITS = [
            (math.radians(-45.0), math.radians(45.0)),  # Joint 1
            (math.radians(-60.0), math.radians(-50.0)),  # Joint 2
            (0.050, 0.200),  # Joint 3
            # (-1.57, 1.57),  # Joint 4
            # (-3.14, 3.14),  # Joint 5
            # (-1.0, 1.0)  # Joint 6
    ...
    def dithering(self):
        ...
        print('  amplitude: {} rad'.format(self.dith_ampl))
        input('  velocity: {} rad/s'.format(self.dith_ampl * 2.0 * math.pi * self.dith_freq))
        ...
        while True:
            ...
            vel = np.zeros_like(goal)
            vel[self.joint_index] = self.dith_ampl * 2.0 * math.pi * self.dith_freq * cosine + v_offset
            # vel_hist.append(vel[self.joint_index])

            self.arm.servo_jp(goal, vel)

            if t > 10.0: self.dith_off = True
